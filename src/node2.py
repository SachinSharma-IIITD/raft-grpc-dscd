...
class Raft(raft_pb2_grpc.RaftServicer):
    ...
    def RequestVote(self, request, context):
        ...
        if self.voted_for is None or self.voted_for == request.candidate_id:
            ...
            if (len(self.log) == 0 or self.log[-1].term < request.last_log_term) or (self.log[-1].term == request.last_log_term and self.log[-1].index <= request.last_log_index):
                ...
                self.election_timeout.restart_timer()
                # print(
                #     f'Leader lease time acquired: {self.leader_lease_timeout.Timer}')
        ...
        if request.term == self.current_term:
            ...
            if (len(self.log) > request.prev_log_index):
                if not len(self.log) or (request.prev_log_index == -1 or self.log[request.prev_log_index].term == request.prev_log_term):
                    ...
                    self.election_timeout.restart_timer()
                    self.leader_lease_timeout.restart_timer(timeout=request.leader_lease)
                    print(
                        f'{time.time()} Leader lease time acquired: {request.leader_lease} crnt_term = {self.current_term}')
    ...
    def heart_beat_parallel(self, IP_ADDRESS):
        try:
            ...
            print(
                f"{self.leader_lease_timeout.Timer} lease time sending, cur_term = {self.current_term}")
            # try:
            ...
            append_entries_rpc = AppendEntriesReq(
                ...
                leader_lease=self.leader_lease_timeout.Timer)
    ...
    def send_heart_beats(self):
        ...
        print(f"{time.time()} I am now a leader: term = {self.current_term} {self.leader_lease_timeout.Timer} lease time")
    ...
    def leader_wait_thread_event(self):
        ...
        self.leader_lease_timeout.countDown(
            timeout=self.follower_end_leader_lease.Timer, annotation="leader lease")
    ...
    def become_candidate(self):
        ...
        if (self.current_state == "leader"):
           self.leader_lease_timeout.countDown(timeout=self.leader_lease_time_acquired, annotation="leader lease")
           self.send_heart_beats()
        else:
    ...
    def send_vote_request(self, IP_ADDRESS):
        try:
            ...
            self.leader_lease_time_acquired = max(
                response.leader_lease, self.leader_lease_timeout.Timer)
            print(
